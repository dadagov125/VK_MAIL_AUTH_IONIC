window.MAILRU = {
  AppId: false,
  SecretKey: "1ed37638bd4085579fe78c87d7cd2a9b",
  PrivateKey: "0161d35c2122138ef83a7fde8886fa76",

  init: function (options) {
    if (options && options.apiId) MAILRU.AppId = options.apiId
  },
  Auth: {
    Session: false,
    login: function (cd) {
      if (!MAILRU.AppId) return;
      var url = "https://connect.mail.ru/oauth/authorize?client_id=759154&display=mobile&response_type=token&redirect_uri=http://localhos";
      ...
      var active = window.open(url, "_blank", features);

      active.addEventListener('loadstop', function (event) {
        var split = (event.url).split("#");
        var url = split[0];

        if (url == 'http://connect.mail.ru/oauth/success.html') {
          ...
          params = "app_id=" + MAILRU.AppId + "method=users.getInfo" + "session_key=" + session_key;

          var sig = MAILRU.MD5(uid + params + MAILRU.PrivateKey);
          url = "http://www.appsmail.ru/platform/api?method=users.getInfo&app_id=" + MAILRU.AppId + "&session_key=" + session_key + "&sig=" + sig;
    ...
    logout: function (cd) {
      var url = "https://auth.mail.ru/cgi-bin/logout?app=1";
      var xhr = new XMLHttpRequest();
      xhr.open("GET", url);
      xhr.onload = function () {
        console.log("onload");
